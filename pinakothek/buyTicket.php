<?php
    ...
    if($museum == 'Choose...') {
        echo "<script>alert('Please select a museum.');window.location.replace(\"http://localhost:8080/ticket.html\");</script>";
        exit;
    }

    if(!$old_date) {
        echo "<script>alert('Please select a date.');window.location.replace(\"http://localhost:8080/ticket.html\");</script>";
        exit;
    }
    ...
    while($i > 0) {
        ...
        if(($row2['total']-$row2['sold']) < $amount) {
            echo "<script>alert('Please check the amount. There is no available ticket.');window.location.replace(\"http://localhost:8080/ticket.html\");</script>";
            exit;
        }
    ...
    if ( $success == true ) {
        ...
        echo "<script>alert('Error happened during purchasing ticket.');window.location.replace(\"http://localhost:8080/ticket.html\");</script>";
    }
    ...
    //-------------------------------------------location url change---------------------------------------------------------
    echo "<script>alert('You paid {$paid}$.');window.location.replace(\"http://localhost:8080/ticket.html\");</script>";
